...
export function prefixMap(detailed = false, selected = "all") {
  ...
  return `<section class="map-panel prefix-map ${detailed ? "detailed-map" : ""}" data-detailed="${detailed}"><div class="map-heading"><h2>市外局番・都道府県マップ</h2><span>全59パターン · 色は先頭2桁</span></div><div class="prefix-layout"><div class="prefix-visual" tabindex="0" role="region" aria-label="全国図。小画面では左右にスクロールできます">${mapSvg(active, true)}</div><div class="prefix-info"><div class="prefix-legend" role="group" aria-label="先頭2桁で地図を絞り込む"><button data-prefix="all" aria-pressed="${selected === "all"}">すべて</button>${prefixGroups.map((group) => `<button data-prefix="${group.code}" aria-pressed="${selected === group.code}" style="--prefix:${group.color}"><span></span>${group.code}</button>`).join("")}</div><p class="map-instruction">各県に対応する学習パターンをすべて表示しています。色は先頭2桁に対応します。下の色付き番号を選択すると対応する県を強調表示します。</p><div class="prefix-correspondence" aria-live="polite">${detailed || selected !== "all" ? groups.map((group) => `<div><b style="--prefix:${group.color}">${group.code}</b><span>${[...new Set(entries.filter((entry) => entry.code.startsWith(group.code)).flatMap((entry) => entry.prefectures))].join("・")}</span></div>`).join("") : ""}</div></div></div><p class="map-source">提供資料の59パターンに基づく対応図。県境をまたぐ番号も含みます。県内の利用区域・地形・縮尺を示す図ではありません。</p></section>`;
}
