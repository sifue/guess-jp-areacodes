...
function shell(content) {
  app.innerHTML = `<header class="header"><a class="brand" href="#home"><span class="brand-icon">局</span>局番ノート<span class="brand-sub">日本の市外局番をおぼえる</span></a><nav aria-label="メインナビゲーション"><a href="#home" ${state.page === "home" ? 'aria-current="page"' : ""}>ホーム</a><a href="#study" ${state.page === "study" ? 'aria-current="page"' : ""}>暗記ノート</a><a href="#history" ${state.page === "history" ? 'aria-current="page"' : ""}>学習記録</a></nav></header><main id="main" tabindex="-1">${content}</main><footer><a class="footer-brand" href="#home">局番ノート <span>数字の向こうに、見覚えのある場所を。</span></a><div><a href="#about">使い方・出典</a><span>© ${new Date().getFullYear()} Soichiro Yoshimura <a href="https://github.com/sifue">@sifue</a></span><span>v${version}</span></div></footer>`;
}
function home() {
  shell(
    `<section class="hero"><div class="hero-copy"><div class="eyebrow"><span></span> GEOGUESSR のための、ちいさな地理学習</div><h1>数字から、<br>日本を<span class="underline">思い出す。</span></h1><p class="intro">看板に見つけた「市外局番」が、場所のヒントになる。<br class="desktop">地域のつながりと覚え方で、少しずつ日本を頭の中に。</p><button class="primary" data-start="ten">まずは10問、挑戦する <span>↗</span></button><div class="hero-caption">登録不要 <i>·</i> 1回 約2分 <i>·</i> スマホでも気軽に</div><div class="hero-facts"><div><strong>47<span>都道府県</span></strong><small>日本をまるごと</small></div><div><strong>${entries.length}<span>パターン</span></strong><small>先頭2〜3桁で学ぶ</small></div><div><strong>3<span>つのモード</span></strong><small>自分のペースで</small></div></div></div><div class="map-panel"><div class="map-heading"><span>NUMBER → PLACE</span><span>日本 市外局番マップ</span></div>${mapSvg()}<div class="map-note"><span class="note-number">01<span>系</span></span><div><strong>北から南へ、数字で旅する。</strong><p>まずは地方のまとまりをつかもう。</p></div><span class="note-end">09<span>系</span></span></div></div></section><section class="mode-section"><div class="section-heading"><div><span class="eyebrow">LET’S PRACTICE</span><h2>今日の学び方を選ぼう</h2></div><span class="muted">まずは10問。慣れたら、全国へ。</span></div><div class="mode-grid"><button class="mode-card recommended" data-start="ten"><span class="recommend">はじめての方におすすめ</span><span class="card-top"><span class="mode-icon">10</span><span class="card-index">01 / QUICK QUIZ</span></span><h3>市外局番10問モード</h3><p>ランダムな10問で、気軽に腕試し。<br>覚え方を確認しながら、知識をひとつずつ。</p><span class="card-bottom"><span>10問 <i>·</i> 4択 <i>·</i> 約2分</span><b>→</b></span></button><button class="mode-card" data-start="all"><span class="card-top"><span class="mode-icon ochre">全</span><span class="card-index">02 / FULL CHALLENGE</span></span><h3>全市外局番モード</h3><p>資料にある${entries.length}パターンをひと巡り。<br>得意な地域も、苦手な地域も見つけよう。</p><span class="card-bottom"><span>${entries.length}問 <i>·</i> 4択 <i>·</i> じっくり</span><b>→</b></span></button><a class="mode-card" href="#study"><span class="card-top"><span class="mode-icon blue">▤</span><span class="card-index">03 / MEMORY NOTE</span></span><h3>全暗記モード</h3><p>地方ごとの番号と覚え方を、ひと目で。<br>答えを隠して、思い出す練習も。</p><span class="card-bottom"><span>一覧 <i>·</i> 覚え方 <i>·</i> 表示／非表示</span><b>→</b></span></a></div></section><section class="learning-strip"><div class="strip-title"><span>✳</span><h3>覚えるコツは、<br>「つながり」で見ること。</h3></div><div><b>01 <span>地方でまとめる</span></b><p>01系は北海道、09系は九州。<br>大きなまとまりから覚えよう。</p></div><div><b>02 <span>数字を並べる</span></b><p>17青森 → 18秋田 → 19岩手。<br>近くの番号をセットにしよう。</p></div><div><b>03 <span>間違いをもう一度</span></b><p>答え合わせの覚え方がヒント。<br>復習で記憶を確かめよう。</p></div></section><p class="scope-note">学習対象は資料の先頭2〜3桁です。複数の都道府県にまたがる番号は、県の組み合わせで出題します。正式な全市外局番の網羅一覧ではありません。</p>`,
  );
}
...
function quiz() {
  ...
  shell(
    `<div class="narrow"><div class="page-top"><a href="#home">← モード選択へ</a><span>${modeName(state.mode)}</span></div><div class="quiz-progress"><strong>問題 ${state.index + 1}<span> / ${state.questions.length}</span></strong><span>${state.answers.filter((a) => a.correct).length}問正解</span></div><progress value="${state.index + (answered ? 1 : 0)}" max="${state.questions.length}" aria-label="回答済みの問題数"></progress><section class="question-card"><span class="eyebrow">この市外局番はどの都道府県ですか？</span><div class="question-number">${q.code}</div><p class="muted">先頭${q.code.length}桁${q.prefectures.length > 1 ? " · 該当する" + q.prefectures.length + "都道府県の組み合わせを選択" : " · 該当する都道府県を選択"}</p><div class="choices">${state.options.map((option, i) => `<button data-answer="${i}" ${answered ? "disabled" : ""} class="choice ${answered && option === label(q) ? "correct" : ""} ${answered && option === state.selection && !correct ? "incorrect" : ""}"><span class="choice-key">${i + 1}</span><span>${option}</span>${answered && option === label(q) ? "<b>✓</b>" : ""}${answered && option === state.selection && !correct ? "<b>×</b>" : ""}</button>`).join("")}</div>${answered ? `<div class="feedback ${correct ? "" : "wrong"}" role="status"><strong>${correct ? "✓ 正解！" : "↻ あと一歩。ここで覚えよう。"}</strong><p>正解：${label(q)}</p><div class="memory-hint"><span>覚え方</span>${q.hint}</div><details><summary>地図で場所を確認</summary>${mapSvg(q.prefectures)}</details></div><button class="primary next" data-next>${state.index + 1 === state.questions.length ? "結果を見る" : "次の問題へ"} →</button>` : '<p class="keyboard-hint">選択肢をタップして回答 <span>／ キーボード 1〜4</span></p>'}</section><p class="scope-note">${answered ? "覚え方を確認して、自分のペースで次へ。" : "焦らず、思い出してみよう。時間による減点はありません。"}</p></div>`,
  );
}
function answer(index) {
  ...
  state.answers.push({
    ...
    seconds: (performance.now() - state.started) / 1000,
  });
  render();
...
function historyPanel(mode) {
  ...
  return `<div class="stats"><div><small>プレイ回数</small><strong>${records.length}<span>回</span></strong></div><div><small>最高スコア</small><strong>${best}<span>pt</span></strong></div><div><small>最高スコア最速タイム</small><strong>${fastest ? time(fastest.duration) : "—"}</strong></div></div>${
    records.length
      ...
      : '<div class="empty">まだ学習記録がありません。最初の10問から始めよう。</div>'
  }`;
}
function result() {
...
function study() {
  shell(
    `<section class="study-page"><div class="page-title"><span class="eyebrow">YOUR MEMORY NOTE</span><h1>覚えて、隠して、思い出す。</h1><p>番号と場所を、覚え方でつなぐ全暗記モード。</p></div><div class="study-toolbar"><label class="search"><span>⌕</span><input id="search" type="search" placeholder="市外局番・都道府県を検索" aria-label="市外局番・都道府県を検索" value="${escapeHtml(state.filter)}"/></label><button class="secondary" data-hide-all>すべて隠す</button><button class="secondary" data-show-all>すべて表示</button></div><div class="region-tabs" role="group" aria-label="地域で絞り込み"><button data-region="all" aria-pressed="${state.region === "all"}">全国</button>${regions.map((r, i) => `<button data-region="${i}" aria-pressed="${state.region === String(i)}"><span style="background:${r.color}"></span>${r.name}</button>`).join("")}</div><p class="scope-note">先頭2〜3桁の学習表です。越境する番号は、複数県の対応をまとめて覚えます。「すべて隠す」は番号を残し、県名と覚え方を隠します。</p><div id="study-list"></div><details class="prefecture-table"><summary>都道府県から番号を探す（47都道府県の対応表）</summary><div class="table-wrap"><table><thead><tr><th>都道府県</th><th>先頭2〜3桁</th></tr></thead><tbody>${regions
  ...
  document.querySelector("#study-list").innerHTML =
    `<p class="list-count" role="status">${filtered.length} / ${entries.length}パターン</p><div class="study-grid">${filtered
      .map((q) => {
        ...
        return `<article class="study-card" style="--region:${regions[regionOf(q.prefectures[0])].color}"><div class="study-card-top"><b>${q.code}</b><button class="reveal" data-toggle="${q.code}" aria-expanded="${!hidden}" aria-label="${q.code}の答えを${hidden ? "表示" : "隠す"}">${hidden ? "表示する ＋" : "隠す −"}</button></div>${hidden ? '<div class="hidden-answer">どの都道府県？<small>頭の中で答えてから、表示しよう。</small></div>' : `<h3>${label(q)}</h3><p>${q.hint}</p>`}</article>`;
      })
      .join(
        "",
