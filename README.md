...
数字から、日本を思い出す。GeoGuessrの日本マップ向けに、市外局番の先頭2〜3桁と都道府県の対応を学ぶ静的Webサイトです。制作者：**Soichiro Yoshimura @sifue**。バージョン：**1.3.3**。
...
全国図は高さ制限を設けず、図内の縦スクロールなしで全体を表示します。小画面では横方向だけスクロールできます。

全暗記モードの冒頭にもトップと同じ全国図を表示します。全47県の対応パターンを省略せず表示し、最小幅1,000px・高さ制限なしで共通化しています。
